...
class UserIndexItem extends React.Component {
  ...
  render() {
    ...
    const followingItem = followings.filter(following => following.receiverId === user.id && followings[i].authorId == currentUser.id)[0]
    const following = Object.assign({}, {author_id: currentUser.id, receiver_id: user.id})
    
    let followStatus = false;

    for (let i = 0; i < followings.length; ++i) {
      if (followings[i].receiverId === user.id && followings[i].authorId === currentUser.id) {
        followStatus = true;
        break;
      }
    }
